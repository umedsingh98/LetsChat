    ...
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
          ...
          messagesDummyData.map((msg, index)=> (
            <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f5116f10f3cd28382ed02' && 'flex-row-reverse'}`}>
              ...
              <div className="text-center text-sm">
               ...
               <p className="text-xs text-gray-500">{formatMsgTime(msg.createdAt)}</p>
              </div>
            </div>
          ))
        }
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
          <input type="text" placeholder="Write a message..." className="flex-1 p-3 text-sm border-none outline-none rounded-lg text-white placeholder:text-gray-400"/>
